...
**Flux de retour :**
Le même principe inverse s'applique, avec une vérification supplémentaire : seul l'emprunteur original peut retourner le livre (403 sinon).
...
| **Middlewares**| Authentification JWT, RBAC, gestion d'erreurs            | `src/middlewares/*.ts`          |
...
```bash
# 1. Lancer la stack complète (4 services)
...
|-------------------|-------|---------------------------|
| library-service   | 8080  | `GET /health` (wget)      |
...
└──────┘    └──────┘    └───────┘    └──────┘    └────────┘
  ESLint      Unit       Docker       GHCR       kubectl
  tsc         Integ.     Trivy        Tags       rollout
  CodeQL      Coverage   SARIF        sha-xxx
```
...
### 5.6 Job 5 — Déploiement Kubernetes
...
steps:
  ...
  - kubectl set image deployment/library-service \
      library-service=ghcr.io/.../projet-microservice-archi:sha-<commit>
  - kubectl rollout status deployment/library-service --timeout=120s
```
...
- **HTTP failed 12.96%** : ces requêtes corresponsent aux tentatives d'emprunt/retour qui retournent des codes 4xx attendus (livre déjà emprunté, pas l'emprunteur original). Ce ne sont pas des erreurs serveur mais des réponses métier correctes.
...
### 7.8 Règles d'alertes Prometheus
...
- **Monitoring** : stack Prometheus + Grafana auto-provisionnée avec 6 panneaux et 3 règles d'alertes
- **CI/CD** : pipeline GitHub Actions en 5 étapes avec scan de sécurité (CodeQL + Trivy)
- **Performance** : 391 req/s confirmé par k6 avec 0% d'erreur et P95 < 500 ms

### 9.2 Perspectives d'amélioration

**Court terme :**
- **Rate limiting** : ajouter `express-rate-limit` pour protéger les endpoints d'authentification contre le brute-force
...
- **Distributed tracing** : intégrer OpenTelemetry pour le suivi des requêtes à travers plusieurs services
